...
magnification at the location of the quasar images is calculated by solving
the lens equation using \texttt{gravlens} \cite{2011ascl.soft02003K}. The
...
luminosity AGN \citep[e.g.,][]{Bennert++11}, and have realistic colors for AGN at the
...
fainter system, we %elected
have chosen to make the source galaxies artificially
fainter ($K$=26) in order to avoid saturating the central pixels while
...
\end{eqnarray}
%
The amplitude $I_{\mathrm{e}}$ is the intensity at the effective radius
...
the axis ratio. The dependence of $k$ with $n$ is given with high accuracy by
 \textbf{Ciotti \& Bertin (1999?).} Empirically, $n$ increases with galaxy
luminosity, most galaxies being fitted by S\'ersic index in the range
$0.6 \le n \le 10.0$ \cite{2006AJ....132.2685M}. We adopt $n=4$ for
the deflectors and n=4/3 for the sources, consistent with typical observed values.
...
quasar images, which are %therefore
 added directly to the source plane.
 Finally, the light models models are convolved with the PSF appropriate for each instrument.
...
between \textbf{X and Y seconds per system, XLM to specify}) on a state of the
...
for each one using the python Markov Chain Monte Carlo sampler \texttt{pymc}.
The results are summarized in figures\textbf{.. X to Y..}
Table~\ref{tab:exptimes} summarizes the exposure-time requirements for HST, JWST, Keck LGSAO and NGAO
 in order to reach 2\% uncertainties on $\gamma^{\prime}.$ \textbf{Should we provide some summary for TMT etc
  using a different kind of metric?}
...
Magnitudes (in the ABmag system) and magnifications of multiple images.
 The source magnitude `mag' is given in the source plane so that the observed magnitude in the image plane is given by
  $\mathrm{mag}-2.5\log_{10} \mu$, where $\mu$ is the magnification of image. \\
$^a$ This configuration yields 4 QSO images. \\
$^b$ This configuration yields 2 QSO images. \\
...
\hline
  ...
  NGAO   & $18\times10^3$s & 180 s  & $3\times10^3$s & 150 s \\
\hline
\hline
\end{tabular}
\begin{tablenotes}
\item 
Exposure times required to attin a 2\% uncertainty on the inferred $\gamma^\prime.$
 With 360 s of exposure, the TMT already enables percent-level precision and accuracy.
 Euclid, WFIRST and LSST are not quoted here because their exposure times cannot be tuned arbitrarily.\\
\end{tablenotes}
\label{tab:exptimes}
\end{center}
\end{table*}
