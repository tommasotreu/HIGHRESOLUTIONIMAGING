...
Table~\ref{tab:telescopes parameters} and figure~\ref{fig:zp_wavelength} display the main properties of the telescopes that are needed when generating images of mock gravitational lenses. Figure~\ref{fig:PSF_montage}\textbf{[XLM, can you do it? Sure! have done!]} shows the typical PSF of each instrument.
    ...
    1. The instrument properties of HST used are taken from Space Telescope Science Institute and HST Exposure Time Calculator (\url{http://etc.stsci.edu}). We are interested in F814W band. The pixel size is $0.05^{\prime\prime}$/pixel.
    
    2. We determinate JWST instrument properties by JWST Exposure Time Calculator (\url{http://jwstetc.stsci.edu}). F200W band is selected to considered. The pixel size is $0.032^{\prime\prime}$/pixel. Zero point, readout noise, and background noise are calculated from count rates at fixed exposure time.
    
    3. According to the informations given by Keck NIRC2 Observer's Manual (\url{http://www2.keck.hawaii.edu/inst/nirc2/Manual/ObserversManual.html#Section2.2}), properties of Keck can be deduced, both with LGSAO and NGSAO. Here, we choose MCDS as the readout mode of the detector, meanwhile read number N = 16. 
    
    4.  The selected filter's zero point and background noise of TMT is obtained according to a prospect work (Do et al. 2014 \cite{2014AJ....147...93D}). The pixel size is $0.004^{\prime\prime}$/pixel. Using TMT ETC (\url{http://tmt.mtk.nao.ac.jp/ETC_readme.html}), readout noise can be calculated (please note that the total efficiency is around 0.4). 
